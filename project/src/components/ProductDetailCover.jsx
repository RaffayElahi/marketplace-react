...
import ProductDetailCoverLoader from "./Loaders/ProductDetailCoverLoader";
...
const findVariantIdByColorAndSize = (product, color, size) => {
  ...
  if (variant.length === 0) {
    ...
    for (let i = 0; i < req.length; i++) {
      const variant = product.variants.filter(
        (variant) =>
          variant.size === req[i].size && variant.color.toLowerCase() === color
      );
      if (variant.length !== 0) {
        return {
          id: variant[0]._id,
...
export default function ProductDetailCover({ productCode }) {
  const {
    ...
    error,
    isLoading,
  } = useQuery(["product", productCode], () => fetchProduct(productCode), {
    staleTime: 10 * 60 * 1000,
    retry: false,
  });

  if (isLoading) return <ProductDetailCoverLoader />;
  if (error?.message === "Product not found") return;
    ...
    <div className="w-full h-auto flex flex-col border-b pb-5 border-black lg:flex lg:flex-row">
      ...
      <div className="flex flex-col p-6 gap-7 lg:justify-center lg:pl-16 lg:pt-16 lg:pr-16 lg:w-1/2">
        <div className="space-y-2">
          <h1 className="text-black text-3xl uppercase font-[500]">
            {product.name}
          </h1>
          <h1 className="text-black text-3xl uppercase font-[500]">
        ...
        <div className="">
          <h1 className="text-current text-3xl uppercase font-[500]">
            ${product.variants[0].price}{" "}
            <span className="font-[300]">(TAX INCLUDED)</span>
          </h1>
        </div>
        <div className="w-full flex gap-5 justify-center flex-col">
          <span className="uppercase text-sm text-left">
        ...
        <span className="uppercase text-sm text-left">
          Note: Scroll the carousel to see other color variant of this product.
        </span>
        <div className="w-full flex gap-5 justify-center flex-col">
        ...
        {product.variants ? (
          <div className="w-full my-10">
            <Cart
              ...
              Usersize={Usersize}
              product={product}
              filterFunction={findVariantIdByColorAndSize}
            />
          </div>
        ) : (
          ""
